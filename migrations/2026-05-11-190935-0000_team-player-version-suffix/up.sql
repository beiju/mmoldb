alter table data.team_player_versions
    add column name_suffix text; -- null = no suffix
...
begin
    ...
    where tpv.mmolb_team_id = NEW.mmolb_team_id
      ...
      and tpv.name_suffix is not distinct from NEW.name_suffix
...
create function player_full_name(player data.team_player_versions) returns text
as $$
begin
