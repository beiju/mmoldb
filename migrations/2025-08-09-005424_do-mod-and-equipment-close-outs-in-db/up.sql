-- this adds logic that whenever we insert a new version, we:
-- 1. close out any modifications for this player whose index is
--    higher than the new player version's num_modifications
-- 2. close out any equipment version for this player whose slot
--    is no longer present in occupied_equipment_slots
-- 3. close out any equipment effect version for this player whose
--    slot is no longer present in occupied_equipment_slots
...
begin
    ...
    where mmolb_player_id = NEW.mmolb_player_id
        and modification_order >= NEW.num_modifications
...
-- this adds logic that whenever we insert a new version, we:
-- 1. close out any effects for this player whose index is
--    higher than the new equipment version's num_effects
...
begin
    ...
    -- ...and close out any effects that are past the end of the currently-valid version...
    update data.player_equipment_effect_versions
    set valid_until = NEW.valid_from
    where mmolb_player_id = NEW.mmolb_player_id
        and equipment_slot = NEW.equipment_slot
        and effect_index >= NEW.num_effects
        and valid_until is null;
...
-- out-of-slot equipment effects should already be handled by the same index that handles
-- past-the-end equipment effects
create index close_out_of_slot_equipment on data.player_equipment_versions (mmolb_player_id, equipment_slot)
