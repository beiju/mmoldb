-- we'll re-add this later
drop materialized view if exists data.player_versions_extended;

-- it's not practical to migrate all the data to the new format
truncate table
    data.modifications,
...
create table taxa.modification_type (
...
create table data.player_modification_versions (
    ...
    modification_type bigint references taxa.modification_type not null,
    ...
    unique (mmolb_player_id, modification_type, modification_index, valid_from),
    unique nulls not distinct (mmolb_player_id, modification_type, modification_index, valid_until)
);

-- don't need to drop the trigger, it got dropped when we dropped the table
...
begin
    ...
    where pmv.mmolb_player_id = NEW.mmolb_player_id
      ...
      and pmv.modification_type is not distinct from NEW.modification_type
    ...
    if FOUND then
        ...
        where mmolb_player_id = NEW.mmolb_player_id
          and modification_type = NEW.modification_type
...
alter table data.player_versions drop column lesser_boon;
alter table data.player_versions drop column greater_boon;
